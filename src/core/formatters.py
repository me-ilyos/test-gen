...
def create_word_document(json_data: Dict, output_path: str) -> None:
    ...
    for question in json_data["questions"]:
        ...
        row = 2
        incorrect_variants = [v for v in question["variants"] if v["id"] != question["correct"]]
...
def create_student_format_word(json_data: Dict, output_path: str, include_variants: bool = True) -> None:
    """
...
def create_duplicate_report_word(report_text: str, output_path: str) -> None:
    ...
    for line in report_text.split('\n'):
        if line.startswith("SIMILAR QUESTIONS FOUND") or line.startswith("SIMILAR ANSWER VARIANTS FOUND"):
